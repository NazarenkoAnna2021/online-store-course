...
const generateJwt = (id: number, email: string, role: string) => {
    return jwt.sign({ id, email, role }, String(process.env.SECRET_KEY), { expiresIn: '24h' });
}
...
class UserController implements IUserController {
    registration = async (req: Request, res: Response, next: NextFunction) => {
        ...
        const token = generateJwt(id, email, user.getDataValue('password'));
    ...
    login = async (req: Request, res: Response, next: NextFunction) => {
        ...
        const token = generateJwt(user.getDataValue('id'), email, user.getDataValue('password'));
